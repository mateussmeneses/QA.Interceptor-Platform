# QA.Interceptor — Frontend Backlog (Planning Catalog)

> Status oficial foi movido para `docs/backlog/BACKLOG_CANONICAL.md`.
> Este arquivo permanece como catalogo de escopo frontend.
> Coluna "Status" neste arquivo representa estado de planejamento de UX/componente, nao status oficial de implementacao.
> Priorizacao de execucao deve seguir `BACKLOG_CONSOLIDATED.md`.
...
| **RUL-UI-003** | Rule editor modal skeleton            | DEVELOPER | P0       | L      | In Progress | Form layout: name / type / priority / condition / payload; tabs for advanced; not wired to backend yet |
...
| **HST-UI-003** | Evidence export dialog        | DEVELOPER | P0       | M      | In Progress | Format selector (JSON/Markdown/HTML) + preview + download button            |
