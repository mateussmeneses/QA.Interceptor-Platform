...
import { detectConflicts, groupConflictsByRule, sortConflictsBySeverity } from "./conflict-detector";
...
const makeRule = (overrides: Partial<Rule> & { id: string; name: string; type: Rule["type"] }): Rule => ({
  priority: 10,
  enabled: true,
...
describe("detectConflicts — no conflicts", () => {
  ...
  it("no conflict when URL patterns are disjoint", () => {
    const rules = [
      makeRule({ id: "r1", name: "A", type: "mock-status", condition: { urlContains: "/api/users" } }),
      makeRule({ id: "r2", name: "B", type: "mock-status", condition: { urlContains: "/api/orders" } }),
    ];
    const report = detectConflicts(rules);
    expect(report.conflicts).toHaveLength(0);
  });
  ...
  it("no conflict for rules of different dimensions with overlapping URL", () => {
    const rules = [
      makeRule({ id: "r1", name: "A", type: "delay", priority: 5, condition: { urlContains: "/api" } }),
      makeRule({ id: "r2", name: "B", type: "rewrite-header", priority: 10, condition: { urlContains: "/api" } }),
    ];
    const report = detectConflicts(rules);
    expect(report.conflicts).toHaveLength(0);
  });
...
describe("detectConflicts — same-dimension-overlap", () => {
  ...
  it("detects two mock-response rules sharing a URL scope", () => {
    const rules = [
      makeRule({ id: "r1", name: "Mock 1", type: "mock-response", condition: { urlContains: "/api" } }),
      makeRule({ id: "r2", name: "Mock 2", type: "mock-response", condition: { urlContains: "/api/users" } }),
    ];
    const report = detectConflicts(rules);
    expect(report.conflicts.some((c) => c.kind === "same-dimension-overlap")).toBe(true);
  });
  ...
  it("detects cross-type dimension conflict: mock-response vs rewrite-response", () => {
    const rules = [
      makeRule({ id: "r1", name: "Mock", type: "mock-response", condition: { urlContains: "/api" } }),
      makeRule({ id: "r2", name: "Rewrite", type: "rewrite-response", condition: { urlContains: "/api" } }),
    ];
    const report = detectConflicts(rules);
    expect(report.conflicts.some((c) => c.kind === "same-dimension-overlap")).toBe(true);
  });
...
describe("detectConflicts — url-shadow", () => {
  it("detects block rule with no URL condition shadowing a scoped rule", () => {
    const rules = [
      ...
      makeRule({ id: "r2", name: "Mock Users", type: "mock-response", condition: { urlContains: "/api/users" } }),
    ];
    const report = detectConflicts(rules);
  ...
  it("shadow severity is warning", () => {
    const rules = [
      ...
      makeRule({ id: "r2", name: "Rewrite users", type: "rewrite-header", condition: { urlContains: "/api" } }),
    ];
    const report = detectConflicts(rules);
  ...
  it("no shadow when both rules have URL conditions", () => {
    const rules = [
      ...
      makeRule({ id: "r2", name: "Mock /b", type: "mock-response", condition: { urlContains: "/a/b" } }),
    ];
    const report = detectConflicts(rules);
...
describe("detectConflicts — terminal-unreachable", () => {
  ...
  it("no terminal-unreachable if conditions do not overlap", () => {
    const rules = [
      makeRule({ id: "r1", name: "Block /a", type: "block", priority: 1, condition: { urlContains: "/a" } }),
      makeRule({ id: "r2", name: "Mock /b", type: "mock-response", priority: 20, condition: { urlContains: "/b" } }),
    ];
    const report = detectConflicts(rules);
    expect(report.conflicts.every((c) => c.kind !== "terminal-unreachable")).toBe(true);
  });
...
describe("sortConflictsBySeverity", () => {
  ...
  it("does not mutate the original array", () => {
    const input = [
      { kind: "priority-tie" as const, severity: "info" as const, ruleIds: ["a", "b"] as [string, string], ruleNames: ["A", "B"] as [string, string], description: "", suggestion: "" },
      { kind: "terminal-unreachable" as const, severity: "error" as const, ruleIds: ["c", "d"] as [string, string], ruleNames: ["C", "D"] as [string, string], description: "", suggestion: "" },
    ];
    const copy = [...input];
